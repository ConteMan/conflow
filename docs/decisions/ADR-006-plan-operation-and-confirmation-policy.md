...
维护者接受推荐方案 1 及全部配套默认值：

- Plan 与 rollback preview 的默认 TTL 为 15 分钟。
- `Environment.publish.requires_confirmation` 保留为兼容展示字段，确认强度语义由项目级策略承担；待 Spec 015 落地后再评估移除。
- `preview_only` 允许「远端不可达」与「从未配置远端的新项目」两种场景，均不可发布。
- 发布历史默认包含成功与失败记录，以状态徽标区分；失败记录保存 outcome 与 Operation failure。
