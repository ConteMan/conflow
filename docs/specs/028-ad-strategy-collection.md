...
2. 基础 `frequency_policy` 仍是广告位的默认频控；策略在源配置和编译产物中都只保存稀疏覆盖。覆盖字段缺失表示由客户端继承，显式 `null` 表示关闭该约束。
...
| `placement_rule_mode` | enum | 否 | `inherit` 表示全部基础广告位；`allowlist` 表示仅允许白名单 |
| `allowlist_placement_ids` | reference[] | 否 | allowlist 模式可使用该策略的广告位 ID；去重并稳定排序 |
| `frequency_policy_overrides` | object | 否 | key 为基础频控策略 ID，value 为稀疏字段覆盖对象 |

`inherit` 模式要求 allowlist 为空，`allowlist` 模式要求至少一个广告位。`frequency_policy_overrides` 的每个 key 必须命中已有 `frequency_policy`，与广告位范围相互独立。覆盖对象只允许 `cooldown`、`interval`、`max_count`、`shift_count`、`positions`；字段缺失表示客户端继承基础频控，显式 `null` 表示关闭该约束，非空值复用 Spec 020 的领域类型。
...
- allowlist 从 Conflow 广告位 ID 映射为客户端 `client_id`；`inherit` 固定输出空 allowlist；缺失、重复或无法映射均 blocking。
- `frequency_policy_overrides` 只输出显式存在的策略和字段，并以基础 `frequency_policy` ID 为 key；不得展开或复制最终有效频控。
- 客户端按 `placement.frequency_policy_id` 读取基础频控并应用当前策略的同 ID 稀疏覆盖；基础频控字段变化只更新频控聚合参数，不重写策略参数。
- 参数来源包含设置实体、全部策略实体、allowlist 引用的广告位身份及覆盖引用的频控策略身份；广告位 `client_id` 变化可以同时影响 placements 与 strategies 两个远端参数。
- 现有 v2 配置没有设置和策略实体时不输出策略参数，不产生删除或纳管风险。

## 校验、Diff 与风险

- singleton、参数 key、引用、inherit/allowlist 联合约束、覆盖字段白名单与领域值执行 blocking 校验；已有覆盖当前未命中任何适用广告位时产生非阻断 warning。
...
- 新建/编辑进入独立详情页；分区展示基础信息、广告位规则与 allowlist、逐基础频控策略的稀疏覆盖、影响摘要和高级源映射。
- 广告位范围与频控覆盖保持独立编辑；每个基础频控策略展示当前适用范围内的命中广告位数量，零命中覆盖显示非阻断提示。
...
- 缺失引用、inherit 带非空 allowlist、allowlist 为空、未知覆盖字段、重复 `client_id`、key 冲突均 blocking。
- Plan 对基础频控字段变化只追踪频控参数，对 allowlist 广告位 `client_id` 变化追踪 placements 与策略参数，并按基础值与稀疏覆盖合并后的有效频控判定策略放宽/收紧风险。
