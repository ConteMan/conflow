# Spec 015：校验、Plan、发布与回滚 UI

> 状态：待实现  
> 依赖：Spec 008、009、010、011、012、014

## 目标

完成从配置草稿到安全发布的 PM 主流程，并清晰展示诊断、影响、风险、进度、历史和回滚证据。

## 结构前提（Spec 012 评审收口，2026-07-10）

- Plan 采用逐层展开树：业务变更 → 受影响广告位 → 最终写入的远端参数；原始 Firebase diff 内嵌于展开层级，不与业务 diff 并列 tab。
- Production 发布为独立步骤页（审阅 → 确认风险 → 执行），不用 modal。
- 确认要求：输入环境 ID + 高风险项逐项勾选；确认强度为项目级配置项，默认始终输入环境 ID，可配置放宽为「低风险 Plan 仅勾选确认」；高风险项逐项勾选在任何配置下不可关闭。
- 校验入口形态（常驻底栏 vs 独立页）待 Spec 007 诊断量级确认，实现先按独立页 + 全局徽标。
- 各流程的八态定义（入口/正常/空/加载/错误/冲突/危险确认/成功）以 docs/design/ui/flows/ 为准。

## 合同前置

开工前关闭 [`UI-API-014` 至 `UI-API-021`](../design/ui/contract-gaps.md#spec-015-开工门槛)，并确认 `UI-API-004`、`UI-API-011`、`UI-API-012` 已收口。readiness、Plan 生命周期、风险项、确认要求、ETag 恢复、Operation 与回滚预览均以服务端合同为权威，前端不复制判定逻辑。

## 范围

- 校验中心：按实体、严重级别和字段路径聚合，可跳转修复。
- Plan：业务语义 diff、Firebase 参数 diff、影响对象、风险分级和 artifact 下载。
- Provider 拉取/验证进度；SSE 断开自动回退 Operation 轮询。
- Production 发布确认：环境、版本、风险、ETag 和确认要求同时展示。
- 发布成功/失败、版本历史、默认值下载和回滚确认。
- plan 过期、draft revision 冲突、remote ETag 冲突的恢复路径。

## API

消费 Spec 007–011 的 validation、plan、operation、release 和 defaults 端点。

## 非范围

- 多人审批、定时发布、通知中心和组织权限。

## 验收

- E2E 覆盖正常发布、校验失败、计划过期、ETag 冲突、网络失败和回滚。
- 发布按钮只在服务端判定就绪后可进入确认；前端禁用不替代服务端校验。
- 原始 Firebase diff 可查看，但默认首先展示业务语义 diff。
- 用户关闭页面后重新打开，仍可恢复正在运行的 Operation 与最终结果。
