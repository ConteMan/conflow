...
> 维护者修订（2026-07-16）：本节迁移器**裁剪出实现范围，不再实现**（Issue #41 据此关闭）。唯一真实项目直接以 v2 起步并经 Spec 021 受控导入进入，无任何 v1 生产工作区；项目处于内部使用的初级阶段，不考虑存量迁移。本节合同保留为历史设计记录，未来若出现真实 v1 存量需求，以此为基础另行立项。
