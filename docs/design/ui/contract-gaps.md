...
| `UI-API-009` | 字段 caption 还要表达“线上当前值”（评审记录 `2026-07-10-structure-directions.md:47`；Spec 014:14）。 | 已降级（014 首版）：编辑器 caption 不含线上值；009 仍保护完整 Firebase 快照，尚无 provider-neutral 的受管理字段远端投影。 | 由 008/009 收口：明确远端对照在 Plan 或只读 remote projection 中的展示；若回到字段 caption，必须返回脱敏、可映射到字段 path 的远端值摘要。 | 008 / 009 / 014 |
