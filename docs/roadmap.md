...
| Spec | 方向 | 完成标准 |
|---|---|---|
| [020](specs/020-mobile-ad-pack-v2.md) | 通用移动广告 Pack v2、聚合参数编译和显式 v1 迁移 | v1 行为保持不变；v2 可确定性编译独立开关参数与版本化聚合参数，并复用现有发布安全边界 |
| [028](specs/028-ad-strategy-collection.md) | 可手动管理的通用广告策略集合 | 不配置策略时保持 v2 兼容；配置后可校验、编译、审阅、发布和回滚版本化策略参数 |

v0.2 的具体任务状态继续由 GitHub Issues 和 Milestone 跟踪。配置导入、Pack 选择/迁移入口与跨环境绑定矩阵可以分别交付，但都必须消费同一份版本化 Pack 合同，不在 Source、Provider 或 UI 中复制业务规则。
