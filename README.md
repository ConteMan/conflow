...
Conflow 是一个 Go 单二进制工具，同时提供 CLI 和本地 Web GUI。它将远程配置表达为广告位、广告策略、频控策略、功能开关、自定义参数等业务对象，而不是让团队直接编辑 Firebase 控制台里的长 JSON——配置的唯一事实源保存在你的本地工作区（可纳入 Git 管理），Firebase 只是发布目标和审计对象。
