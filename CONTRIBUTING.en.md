...
**Reporting issues:** Use [GitHub Issues](https://github.com/ConteMan/conflow/issues). Issue titles and bodies are written in Chinese. Labels: `bug` (wrong behaviour), `design-gap` (diverges from the UI prototype), `enhancement` (improvement within existing scope), `spec-needed` (requires a new Spec before work can start). See [AGENTS.md](AGENTS.md) for the full workflow.
