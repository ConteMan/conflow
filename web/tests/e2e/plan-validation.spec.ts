...
type Mode = "ready" | "drift" | "preview" | "invalid" | "routine" | "stale";

async function mockReleaseAPI(page: Page, mode: Mode = "ready") {
  let planRequests = 0;
  let operationReads = 0;
  const packVersion = mode === "drift" ? "v2" : "v1";
  const packRef = `mobile-ad-monetization/${packVersion}`;
  const planID = mode === "invalid" ? "plan_invalid" : mode === "routine" ? "plan_routine" : mode === "preview" ? "plan_preview" : mode === "drift" ? "plan_drift" : "plan_ready";
  await page.route("**/api/v1/**", async (route) => {
    const request = route.request(); const url = new URL(request.url()); const path = url.pathname; const method = request.method();
    if (path === "/api/v1/bootstrap") return json(route, bootstrap(packRef));
    if (path === `/api/v1/packs/mobile-ad-monetization/versions/${packVersion}`) return json(route, { data: { ref: packRef, name: "mobile-ad-monetization", version: packVersion, description: "广告配置", capabilities: [], schema_version: packVersion === "v2" ? 2 : 1, entity_types: [] }, meta: meta() });
    if (path.match(/^\/api\/v1\/drafts\/[^/]+\/diagnostics$/) && method === "GET") {
      if (mode === "ready" || mode === "drift") return json(route, validation("fresh", "ready"));
    ...
    if (path === `/api/v1/packs/mobile-ad-monetization/versions/${packVersion}/schema`) return json(route, { data: { version: packVersion === "v2" ? 2 : 1, entities: [], migrations: [] }, meta: meta() });
    if (path.match(/^\/api\/v1\/drafts\/[^/]+$/)) return json(route, { data: { environment_id: "staging", pack_ref: packRef, source_revision: "source", dirty: true, dirty_scopes: ["baseline"], baseline: { source: { present: true, value: {} }, draft: { present: true, value: {} }, resolved: { present: true, value: {} }, dirty: true }, environment_override: { source: { present: false }, draft: { present: false }, resolved: { present: false }, dirty: false }, effective: {}, field_states: [], affected_environments: [] }, meta: meta() });
...
test("发布计划显示 Operation 进度和三级展开树", async ({ page }) => {
  ...
  await expect(page.getByText("2 项直接修改 · 0 项远端漂移 · 2 个受影响实体 · 2 个远端参数")).toBeVisible();
...
test("发布计划单独展示受管远端漂移", async ({ page }) => {
  await mockReleaseAPI(page, "drift");
  await page.goto("/#plan");
  await expect(page.getByText("0 项直接修改 · 1 项远端漂移 · 0 个受影响实体 · 1 个远端参数")).toBeVisible({ timeout: 7000 });
  await expect(page.getByText("受管远端漂移", { exact: true })).toBeVisible();
  await page.getByRole("button", { name: /受管远端参数 ad_strategies_config/ }).click();
  await expect(page.locator(".semantic-tree").getByText("ad_strategies_config", { exact: true })).toBeVisible();
});
...
function bootstrap(packRef = "mobile-ad-monetization/v1") { return { data: { project: { id: "photo-editor", name: "Photo Editor", pack_ref: packRef, source_type: "managed-file" }, environments: [{ id: "staging", name: "Staging", kind: "staging", provider: { type: "firebase-remote-config", project_id: "photo-editor-staging" }, publish: { requires_confirmation: false } }], capabilities: { project_edit: true, environment_manage: true } }, meta: meta() }; }
