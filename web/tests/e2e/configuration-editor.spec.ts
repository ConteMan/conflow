...
type Fixture = { entities: { placements: Placement[]; frequency_policies: Array<{ id: string; [key: string]: unknown }> } };
...
async function mockConfigurationAPI(page: Page, mode: "normal" | "conflict" | "validation" = "normal") {
  let revision = 1;
  let dirty = false;
  const placements = fixture.entities.placements.slice(0, 3).map((item) => ({ ...item }));
  ...
  await page.route("**/api/v1/**", async (route) => {
    ...
    if (listMatch && method === "GET") {
      ...
      if (entityType === "frequency_policy") return json(route, { data: fixture.entities.frequency_policies.map((item) => view("frequency_policy", item.id, fieldsOf(item), false)), meta: meta(revision) });
