import { ArrowLeft, ChevronRight, CircleAlert, Link2, LoaderCircle, Plus, Save, Search, ShieldAlert, SlidersHorizontal, Trash2, X } from "lucide-react";
...
export function ConfigurationEditor({ environment, environments, revision, packRef, onRevision }: {
  ...
  const loadList = useCallback(async (signal?: AbortSignal) => {
    ...
    try {
      const [nextSchema, nextPlacements, nextPolicies, nextSwitches, nextDraft, nextBindings] = await Promise.all([
      ...
      setSchema(nextSchema.data); setPlacements(nextPlacements.data); setPolicies(nextPolicies.data); setSwitches(nextSwitches.data); setDraft(nextDraft.data); setBindings(Object.fromEntries(nextBindings));
  ...
  if (route.mode === "detail") {
    return <PlacementDetail key={`${environment.id}:${route.id ?? "new"}`} environment={environment} environments={environments} revision={revision} schema={schema} placementID={route.id} focusBindings={route.section === "bindings"} onBack={() => { setRoute({ mode: "list" }); void loadList(); }} onSaved={(nextRevision, dirty) => { onRevision(nextRevision, dirty); void loadList(); }} />;
  }
  ...
  return <>
    <ConfigurationList tab={tab} onTabChange={setTab} placements={placements} policies={policies} switches={switches} draft={draft} bindings={bindings} environments={environments} revision={revision} loading={loading} error={error} onRetry={() => void loadList()} onOpenPlacement={(id) => setRoute({ mode: "detail", id })} onOpenBinding={(id) => setRoute({ mode: "detail", id, section: "bindings" })} onCreate={() => setRoute({ mode: "detail" })} onOpenPolicy={setEditingPolicy} onDelete={(entity, entityType) => setDeleting({ entity, entityType })} onSwitchSaved={() => void loadList()} />
    <FrequencyDrawer policy={editingPolicy} environment={environment} revision={revision} draft={draft} onClose={() => setEditingPolicy(null)} onSaved={() => { setEditingPolicy(null); void loadList(); }} onDelete={(entity) => { setEditingPolicy(null); setDeleting({ entity, entityType: "frequency_policy" }); }} onOpenReference={openReference} />
...
function ConfigurationList({ tab, onTabChange, placements, policies, switches, draft, bindings, environments, revision, loading, error, onRetry, onOpenPlacement, onOpenBinding, onCreate, onOpenPolicy, onDelete, onSwitchSaved }: {
  tab: EditorTab;
  ...
  return <main className="page-container configuration-page">
    <header className="page-heading configuration-heading"><div><h1>{title}</h1><p>{description}</p></div>{tab === "placement" ? <Button variant="primary" icon={<Plus size={17} />} onClick={onCreate}>新建广告位</Button> : null}</header>
    ...
    {tab === "placement" ? <PlacementTable placements={placements} draft={draft} bindings={bindings} environmentCount={environments.length} loading={loading} onOpen={onOpenPlacement} onCreate={onCreate} onDelete={onDelete} /> : null}
    {tab === "frequency_policy" ? <FrequencyTable policies={policies} loading={loading} onOpen={onOpenPolicy} onDelete={onDelete} /> : null}
    {tab === "feature_switch" ? <FeatureSwitchTable switches={switches} environment={environments.find((item) => item.id === draft?.environment_id) ?? environments[0]} revision={revision} draft={draft} loading={loading} onSaved={onSwitchSaved} /> : null}
    {tab === "unit_binding" ? <BindingOverview placements={placements} bindings={bindings} environments={environments} loading={loading} onOpen={onOpenBinding} /> : null}
  </main>;
}
...
function PlacementTable({ placements, draft, bindings, environmentCount, loading, onOpen, onCreate, onDelete }: {
  placements: EntityView[];
  draft: DraftView | null;
  bindings: BindingLoad;
  ...
  return <>
    ...
    {loading ? <TableSkeleton /> : placements.length === 0 ? <section className="inline-empty"><SlidersHorizontal size={24} /><h2>还没有广告位</h2><p>广告位定义应用内稳定的广告展示位置。</p><Button variant="primary" icon={<Plus size={16} />} onClick={onCreate}>新建广告位</Button></section> : <section className="table-panel entity-table-panel">
      <table className="entity-table"><thead><tr><th>广告位 key</th><th>类型</th><th>启用状态</th><th>频控策略</th><th>加载超时</th><th>绑定完整度</th><th>未发布修改</th><th><span className="sr-only">打开详情</span></th></tr></thead>
        <tbody>{rows.map((placement) => <PlacementRow key={placement.entity_id} placement={placement} bindings={bindings} environmentCount={environmentCount} onOpen={onOpen} onDelete={onDelete} />)}</tbody>
      </table>
...
function PlacementRow({ placement, bindings, environmentCount, onOpen, onDelete }: { placement: EntityView; bindings: BindingLoad; environmentCount: number; onOpen: (id: string) => void; onDelete: (entity: EntityView, entityType: "placement") => void }) {
  const fields = placement.effective.value.fields;
...
function FeatureSwitchTable({ switches, environment, revision, draft, loading, onSaved }: { switches: EntityView[]; environment: Environment; revision: number; draft: DraftView | null; loading: boolean; onSaved: () => void }) {
  ...
  return loading ? <TableSkeleton /> : <section className="switch-list table-panel"><header><strong>{switches.length} 个功能开关</strong><span>默认值为通用值，保存后才会进入未发布修改。</span></header>{switches.map((item) => { const fields = item.effective.value.fields; const key = String(fields.key ?? item.entity_id); return <div className={`switch-row switch-row--${String(fields.risk_level ?? "low")}`} key={item.entity_id}><div><strong>{switchName(key)}</strong><code>{key}</code></div><div className="switch-row-meta"><RiskTag level={String(fields.risk_level ?? "low")} /><span>回滚：{rollbackLabel(String(fields.rollback_method ?? ""))}</span><button className={fields.default_value ? "switch-control switch-control--on" : "switch-control"} type="button" role="switch" aria-label={`切换 ${key}`} aria-checked={Boolean(fields.default_value)} disabled={saving === item.entity_id} onClick={() => void toggle(item)}><span /></button></div></div>; })}{error ? <p className="binding-error switch-error" role="alert">{error}</p> : null}</section>;
}

function BindingOverview({ placements, bindings, environments, loading, onOpen }: { placements: EntityView[]; bindings: BindingLoad; environments: Environment[]; loading: boolean; onOpen: (id: string) => void }) {
  return loading ? <TableSkeleton /> : <section className="table-panel binding-overview"><table><thead><tr><th>广告位</th>{environments.flatMap((environment) => [<th key={`${environment.id}-ios`}>{environment.name} iOS</th>, <th key={`${environment.id}-android`}>{environment.name} Android</th>])}</tr></thead><tbody>{placements.map((placement) => <tr key={placement.entity_id} onClick={() => onOpen(placement.entity_id)}><td><strong>{String(placement.effective.value.fields.key ?? placement.entity_id)}</strong><code>{placement.entity_id}</code></td>{environments.flatMap((environment) => (["ios", "android"] as const).map((platform) => { const binding = bindings[environment.id]?.find((item) => item.effective.value.fields.placement_id === placement.entity_id && item.effective.value.fields.platform === platform); const value = binding?.effective.value.fields.unit_id_ref; return <td key={`${environment.id}-${platform}`} className={value ? "binding-overview-cell" : "binding-overview-cell binding-overview-cell--missing"}><code>{String(value ?? "未绑定")}</code></td>; }))}</tr>)}</tbody></table><footer className="table-footer">点击广告位进入详情中的环境绑定区。</footer></section>;
}

function FrequencyDrawer({ policy, environment, revision, draft, onClose, onSaved, onDelete, onOpenReference }: { policy: EntityView | null; environment: Environment; revision: number; draft: DraftView | null; onClose: () => void; onSaved: () => void; onDelete: (entity: EntityView) => void; onOpenReference: (reference: EntityReference) => void }) {
...
function PlacementDetail({ environment, environments, revision, schema, placementID, focusBindings, onBack, onSaved }: {
  environment: Environment;
  environments: Environment[];
  revision: number;
