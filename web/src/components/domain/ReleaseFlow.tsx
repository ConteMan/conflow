...
function ReviewStep({ plan, environment, onNext }: { plan: Plan; environment: Environment; onNext: () => void }) {
  return <div className="release-flow-layout"><section className="panel semantic-tree"><header className="tree-heading"><div><h2>审阅发布计划</h2><p>{plan.semantic_changes.length} 项直接修改 · {plan.affected_entities.length} 个受影响实体 · {plan.remote_parameter_changes.length} 个远端参数</p></div><span className={`risk-tag risk-tag--${plan.severity === "blocking" ? "high" : plan.severity}`}>{riskLabel(plan.severity)}风险</span></header><SemanticTree plan={plan} /></section><aside className="release-flow-sidebar"><TargetCard environment={environment} planID={plan.plan_id} remoteVersion={plan.remote_snapshot.version} /><RiskPanel plan={plan} /><Button variant="primary" onClick={onNext}>继续确认风险</Button></aside></div>;
}
