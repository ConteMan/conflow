import { Boxes, Cloud, GitBranch, Settings, ShieldCheck } from "lucide-react";
...
export function Overview({ project, selectedEnvironment, pack, validation, draftDirty, revision, onManageProject, onSaveEnvironment }: {
  project: Project;
  ...
  return (
    <main className="page-container">
      ...
      <section className="metric-grid" aria-label="项目状态">
        ...
        <Metric icon={<GitBranch />} label="未发布修改" value={draftDirty === null ? "正在获取" : draftDirty ? "有修改" : "本地无修改"} detail={draftDirty === null ? "正在获取草稿状态" : draftDirty ? "等待校验与发布" : `revision ${revision}`} muted={draftDirty === null} />
      ...
      <ProviderConnectionCard environment={selectedEnvironment} onSaveEnvironment={onSaveEnvironment} />
    </main>
  );
}
